			...
			new ToolInputSchemaContract(
				...
				Validators: [
					...
					new ToolContractValidator("apply-static-filter-shape", "invalid-apply-static-filter", "rule.actions[*]",
						Context: "When rule.actions[*].type is apply-static-filter, the action must provide targetAttribute (a Lookup column name on the entity) and filter (a friendly group with logicalOperation AND or OR, filters[] of leaves, optional backwardReferenceFilters[]). The action must NOT include items. Supported leaf comparisonType tokens: EQUAL, NOT_EQUAL, GREATER, GREATER_OR_EQUAL, LESS, LESS_OR_EQUAL, IS_NULL, IS_NOT_NULL, START_WITH, NOT_START_WITH, CONTAIN, NOT_CONTAIN, END_WITH, NOT_END_WITH. The rootSchemaName is inferred from targetAttribute's reference schema; do not pass it. Schema-aware errors (unknown column path, datatype mismatch, lookup-value-not-guid, missing lookup record, malformed backward reference) are validated in-process by clio and surface as filter.path-unknown, filter.comparison-not-supported-for-datatype, filter.lookup-value-not-guid, filter.lookup-record-not-found, or filter.backward-reference-not-1n.")
				]),
